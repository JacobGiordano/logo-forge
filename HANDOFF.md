...
Issues #11-#21 complete and merged to main, shipped. Binary tracer defaults to tuned potrace-wasm (with fallback); sidebar scrolls independently; both vector-fit sliders work; selection/crop UI (#6-#9) has full Playwright coverage including mobile touch drag-select; Otsu-threshold-zero bug fixed; stale `logo-forge.html` build removed. No open issues right now.

## Decisions made this session

- #21: `logo-forge.html` (a self-contained single-file build from the repo's first commit, never updated since, ~2337 lines of inlined CSS/JS) deleted as a confirmed-dead historical artifact — all real feature work has lived in `index.html`/`js/app.js`/`css/styles.css` since before #6. Also dropped its mention from `CLAUDE.md`'s codebase-structure diagram. Nothing else in the repo referenced it. Full suite still 30/30 passing.
- #20: mobile drag-to-select fixed. `js/app.js`'s `eventToCanvasPoint` now resolves coordinates from `e.touches[0]`/`e.changedTouches[0]`/`e` itself (mouse and touch share one code path); the old inline `mousedown` handler became `onSelectDragStart`, wired to both `mousedown` and `touchstart`, with `touchmove`/`touchend` routed through the existing `onSelectDrag`/`onSelectDragEnd` — no parallel touch-only state machine. `tests/selection.spec.ts`'s touch `test.fail()` tripwire (from #18) and the stale auto-trim `test.fail()` tripwire (made obsolete by #19) were both flipped to real passing tests in the same pass.
